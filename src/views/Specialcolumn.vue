<template>
  <div class="news-page">
    ...
    <section class="n_banner">
      ...
      <div class="top">
        <div class="wp">
          <h3 data-aos="fade-up" data-aos-delay="200">学习专栏</h3>
          <p data-aos="fade-up" data-aos-delay="600">了解华苏学习专栏</p>
        </div>
      </div>
      <section class="list_sj" data-aos="fade-up">
        <div class="wp">
          <ul>
            <li :class="{ on: currentTab === 'learning' }">
              <router-link to="/specialcolumn/learning" title="学习专栏">学习专栏</router-link>
            </li>
            <li :class="{ on: currentTab === 'market' }">
              <router-link to="/specialcolumn/market" title="市场发展">市场发展</router-link>
            </li>
            <li :class="{ on: currentTab === 'quality' }">
              <router-link to="/specialcolumn/quality" title="质量安全">质量安全</router-link>
            </li>
          </ul>
        </div>
      </section>
    ...
    <section id="m_n_nav">
      ...
      <div id="sub_nav_content" :class="{ show: showMobileNav }">
        <dl>
          <dt :class="{ on: currentTab === 'learning' }">
            <router-link to="/specialcolumn/learning" title="学习专栏">学习专栏</router-link>
          </dt>
          <dt :class="{ on: currentTab === 'market' }">
            <router-link to="/specialcolumn/market" title="市场发展">市场发展</router-link>
          </dt>
          <dt :class="{ on: currentTab === 'quality' }">
            <router-link to="/specialcolumn/quality" title="质量安全">质量安全</router-link>
          </dt>
        </dl>
      </div>
    ...
    <section class="n_container">
      <LearningSpecial v-if="currentTab === 'learning'" />
      <MarketDevelopment v-if="currentTab === 'market'" />
      <QualitySafety v-if="currentTab === 'quality'" />
      <!-- <DevelopmentHistory v-if="currentTab === 'history'" /> -->
    </section>
...
import LearningSpecial from '@/components/specialcolumn/LearningSpecial.vue'
import MarketDevelopment from '@/components/specialcolumn/MarketDevelopment.vue'
import QualitySafety from '@/components/specialcolumn/QualitySafety.vue'
...
export default {
  ...
  computed: {
    currentTab() {
      // 从路由参数获取当前标签页，默认为 'CorporateCulture'
      return this.$route.params.tab || 'learning'
    }
  },
  mounted() {
    // 初始化AOS动画
  ...
  methods: {
    ...
    getCurrentTitle() {
      ...
      return titles[this.currentTab] || '学习专栏'
    },
    initAOS() {
      // 确保AOS库已加载
      if (window.AOS) {
