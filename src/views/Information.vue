<template>
  <div class="news-page">
    ...
    <section class="n_banner">
      ...
      <div class="top">
        <div class="wp">
          <h3 data-aos="fade-up" data-aos-delay="200">信息公开</h3>
          <p data-aos="fade-up" data-aos-delay="600">了解华苏最新信息公开</p>
        </div>
      </div>
      <section class="list_sj" data-aos="fade-up">
        <div class="wp">
          <ul>
            <li :class="{ on: currentTab === 'notifications' }">
              <router-link to="/information/notifications" title="通知公告">通知公告</router-link>
            </li>
            <li :class="{ on: currentTab === 'events' }">
              <router-link to="/information/events" title="重大事项">重大事项</router-link>
            </li>
            <li :class="{ on: currentTab === 'talent' }">
              <router-link to="/information/talent" title="人才招聘">人才招聘</router-link>
            </li>
          </ul>
        </div>
      </section>
    ...
    <section id="m_n_nav">
      ...
      <div id="sub_nav_content" :class="{ show: showMobileNav }">
        <dl>
          <dt :class="{ on: currentTab === 'notifications' }">
            <router-link to="/information/notifications" title="通知公告">通知公告</router-link>
          </dt>
          <dt :class="{ on: currentTab === 'events' }">
            <router-link to="/information/events" title="重大事项">重大事项</router-link>
          </dt>
          <dt :class="{ on: currentTab === 'talent' }">
            <router-link to="/information/talent" title="人才招聘">人才招聘</router-link>
          </dt>
        </dl>
      </div>
    ...
    <section class="n_container">
      ...
      <MajorEvents v-if="currentTab === 'events'" />
      <TalentRecruitment v-if="currentTab === 'talent'" />
    </section>
...
import Notifications from '@/components/information/Notifications.vue'
import MajorEvents from '@/components/information/MajorEvents.vue'
import TalentRecruitment from '@/components/information/TalentRecruitment.vue'
...
export default {
  ...
  computed: {
    currentTab() {
      ...
      return this.$route.params.tab || 'notifications'
    }
  },
  mounted() {
    // 初始化AOS动画
  ...
  methods: {
    ...
    getCurrentTitle() {
      ...
      return titles[this.currentTab] || '通知公告'
    },
    initAOS() {
      // 确保AOS库已加载
      if (window.AOS) {
